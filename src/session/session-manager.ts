/**
 * SessionManager — 管理 RingCentral peer → dsh Agent 的映射和生命周期
 *
 * sessionKey 格式: `ringcentral:${accountKey}:${scope}:${peerId}`
 *   - direct:   peerId = sender person id
 *   - group:    peerId = Group DM chat id
 *   - channel:  peerId = Team/Everyone chat id
 *
 * SessionId 由 sessionKey 确定性派生（SHA-256），
 * 保证同一个用户/聊天的消息始终路由到同一个会话，
...
export class SessionManager {
  ...
  exportMarkdown(scope: ChatScope, peerId: string): string {
    ...
    const lines: string[] = ['# RingCentral 会话导出\n', '> session: ' + record.sessionId + '\n'];
  ...
  private sessionKey(scope: ChatScope, peerId: string): string {
    return 'ringcentral:' + this.accountKey + ':' + scope + ':' + peerId;
  }
  ...
  private async composePreset(presetId?: string): Promise<PresetComposition> {
    ...
    try {
      ...
      this.logger.warn(
        'im-ringcentral: preset ' + (presetId ?? '(default)') + ' unavailable: ' + (err instanceof Error ? err.message : String(err)) + ' — using host composition',
      );
      return {};
    }
  }
  ...
  async getOrCreate(
    ...
    if (live) {
      ...
      try {
        ...
        agent = resumed.agent;
        handle = resumed;
        this.logger.info('resumed session: key=' + key + ' preset=' + (agentPreset ?? 'none'));
      } catch {
        const created = await this.agents.create({
          sessionId,
          meta: {
