...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    RingCentralCardController.prototype.resetField = function (path) {
      ...
      this.saveError = false;
      this.publish();
      return this.writeOps([{ op: "unset", path: path }]).then(function (ok) {
        if (!ok) self.saveError = true;
        self.saving = false;
    ...
    // ── 写路径适配（dsh 版本双路径）──
    // dsh >= 0.1.2：connection.api 已移除，写入走 settingsScope 的
    // scope.mutate(ops, revision)（内部管理 revision 围栏与失败恢复读取；
    // 传输失败 reject，Host 拒绝时静默恢复——读回校验兜底判定失败）。
    // dsh <= 0.1.1-rc.2：scope 无 mutate 方法，回退旧 wire
    // connection.api.settings.mutate({ns, ops, expectedRevision})。
    RingCentralCardController.prototype.writeOps = function (ops) {
      var self = this; // NOSONAR — bundle 内统一 ES5 风格回调（与文件其余控制器一致）
      var snapshot = this.scope.getSnapshot();
      var mutate = this.scope.mutate;
      if (typeof mutate === "function") {
        return mutate.call(this.scope, ops, snapshot.revision).then(function () {
          return self.verifyWrite(ops, self.scope.getSnapshot());
        });
      }
    ...
    // 读回校验：Host 是唯一权威。secret 字段 wire 上被 redact，只校验
    // user 层的存在性（值永不回读）；普通字段校验值相等；unset 校验字段
    // 已从 user 层消失。全部满足才视为写入成功。
    RingCentralCardController.prototype.verifyWrite = function (ops, snapshot) {
      var user = snapshot.user;
      for (var i = 0; i < ops.length; i++) { // NOSONAR — bundle 内统一经典 for 风格
        var op = ops[i];
        var current = readPath(user, op.path);
        if (op.op === "unset") {
          if (current !== undefined) return false;
          continue;
        }
        var secret = false;
        for (var j = 0; j < SECRETS.length; j++) { // NOSONAR — bundle 内统一经典 for 风格
          if (pathKey(SECRETS[j].path) === pathKey(op.path)) { secret = true; break; }
        }
        if (secret ? current === undefined : current !== op.value) return false;
      }
      return true;
    };
    ...
    RingCentralCardController.prototype.save = function () {
      ...
      return this.writeOps(ops).then(function (ok) {
        if (!ok) self.saveError = true;
        self.saving = false;
        self.drafts.clear();
        self.publish();
    ...
    function apply(ctx) {
      ...
      // ── locale 词典注册（必须走 ctx.inject，不能一次性 ctx.get）──
      // 行加载顺序不定：locale 行的 apply 被它自己的 inject
      // （settingsScope 等由行序更靠后的 ui-settings 提供）挡住而 pending，
      // 本行只依赖更早可用的服务，apply 往往先执行——此时一次性
      // ctx.get("locale") 拿到 undefined 且不会重试，词典永不注册，
      // 卡片永久显示原始 key（cardTitle…）。
      // ctx.inject(deps, cb) 在服务出现时触发一次（已出现则立即触发），
      // 无竞态；0.1.2 渲染器支持晚注册词典（revision 变更会重渲染卡片）。
      // locale 服务缺失的部署中回调永不执行（与旧行为一致的优雅降级）。
      ctx.inject(["locale"], function (localeCtx) {
        localeCtx.effect(function () {
          try {
            return localeCtx.locale.register(NS, { en: en, zh: zh });
          } catch (err) {
            console.warn("[dsh-ringcentral] locale register failed:", err);
            return undefined;
          }
        }, "dsh-ringcentral: locale dictionaries");
      });
