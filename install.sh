#!/bin/sh
# dsh-ringcentral 一键安装
#
# 走 dsh CLI 的 profile 插件机制：add 自动初始化 profile（首层 dsh-base），
# pnpm 安装后按 dsh.bundle.patch 元数据把本包追加为 bundle 层。
#
# 用法：
#   sh install.sh                                            # 安装默认包最新版本
#   sh install.sh --version 0.1.0                            # 指定版本
#   sh install.sh --pkg @myorg/my-plugin                     # 指定包名
#   sh install.sh --registry https://registry.npmmirror.com  # 指定 npm registry
set -eu

# ── 默认值 ──
PACKAGE="dsh-ringcentral"
VERSION=""
REGISTRY=""
PROFILE="ringcentral"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo
      echo "选项:"
      echo "  --pkg <name>        指定 npm 包名 (默认: dsh-ringcentral)"
      echo "  --version <ver>     指定包版本 (默认: latest)"
      echo "  --registry <url>    指定 npm registry"
      echo "  --profile <name>    指定 dsh profile 名 (默认: ringcentral)"
      echo "  -h, --help          显示帮助"
      exit 0
      ;;
    *)
